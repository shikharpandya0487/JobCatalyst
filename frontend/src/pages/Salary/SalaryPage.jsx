 import React, { useState,logo } from "react";
    ...
    >
      ...
      <div className="container mx-auto p-4">
        ...
        <div className="flex justify-center mb-6">
          <input
            type="text"
            className="search-input px-3 py-2 border border-gray-500 rounded-l-md focus:outline-none"
            placeholder="Your Job Title"
          />
          <button className="search-button px-1 py-2 bg-green-600 text-white rounded-r-md hover:bg-green-600 focus:outline-none">
            Search
          </button>
        </div>
        <div className="flex gap-20">
          <div className="companies w-full sm:w-2/3">
            {companies.map((company) => (
              <div
                key={company.name}
                className="company bg-gray-200 rounded-2xl shadow-md md:h-30 lg:h-40 w-full sm:w-400px md:w-600px mb-6 flex relative"
              >
                <img
                  src={company.logo}
                  alt={logo}
                  className="w-1/5 h-30 rounded-l-xl"
                />
                <div className="flex-1 flex items-center justify-center p-4">
                  <div className="text-center">
                    <div
                      className="company-name text-2xl font-semibold mt-0"
                      style={{ color: theme === "dark" ? "#333" : "#333" }}
                    >
                      {company.name}
                    </div>
                    <div className="company-salary text-green-500 text-2xl mt-2"
                     style={{ color: theme === "dark" ? "#333" : "#333" }}
                    >
                      {company.salary}
                    </div>
                    <div
                      ...
                      style={{ color: theme === "dark" ? "#333" : "#333" }}
                    >
                      {company.employees} employees hired
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="sidebar flex flex-col w-full sm:w-1/3">
            <div className="highest-salary bg-gray-200 rounded-2xl shadow-md p-4 mb-6">
              <h2 className="text-2xl font-semibold mb-4" style={{ color: theme === 'dark' ? '#333' : '#333' }}>Highest Salaries</h2>
              ...
              <h2 className="text-2xl font-semibold mb-4 mt-4" style={{ color: theme === 'dark' ? '#333' : '#333' }}>
                Popular Salaries
              </h2>
              <ul className="list-disc pl-6 mb-4 text-lg text-gray-900">
